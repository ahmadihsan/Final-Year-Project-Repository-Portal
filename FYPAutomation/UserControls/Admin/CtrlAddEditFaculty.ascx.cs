...
namespace FYPAutomation.UserControls
{
    public partial class CtrlAddEditFaculty : System.Web.UI.UserControl
    {
        ...
        protected void AddFaculty(object sender, EventArgs e)
        {
            ...
            using (var fypEntities = new FYPEntities())
            {
                List<User> usr = fypEntities.Users.Where(x => x.Email == txtEmail.Text).ToList();
                if(usr.Count>0)
                {
                    ...
                    faculty.Password = FYPPasswordManager.Encrypt("comsats");//later it will be changed

                    fypEntities.Users.Add(faculty);
                    if (fypEntities.SaveChanges() > 0)
                    {
                        FYPMessage.ShowPopUpMessage("Success", new List<string>() { "Faculty Added Successfully" }, this.Page, true);
